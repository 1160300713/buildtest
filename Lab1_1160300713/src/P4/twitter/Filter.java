...
public class Filter {
	...
	public static List<Tweet> writtenBy(List<Tweet> tweets, String username) {
		...
		for (Tweet oneuser : tweets) {
			if (oneuser.getAuthor() == username) {
				userSTweets.add(oneuser);
			}
		}
		return userSTweets;
	}
	...
	public static List<Tweet> inTimespan(List<Tweet> tweets, Timespan timespan) {
		List<Tweet> userSTweets = new ArrayList<>();
		for (Tweet oneuser : tweets) {
			if (oneuser.getTimestamp().isAfter(timespan.getStart())
					&& oneuser.getTimestamp().isBefore(timespan.getEnd())) {
				userSTweets.add(oneuser);
			}
		}
		return userSTweets;
	...
	public static List<Tweet> containing(List<Tweet> tweets, List<String> words) {
		List<Tweet> userSTweets = new ArrayList<Tweet>();
		String[] text = null;
		for(Tweet oneuser : tweets) {
			text = oneuser.getText().split(" ");
			for (String userword : text) {
				...
				for (String targetWord : words) {	
					if (userword.equalsIgnoreCase(targetWord)) {
						if (!userSTweets.contains(oneuser)) {
							userSTweets.add(oneuser);
						}
					}
				}
